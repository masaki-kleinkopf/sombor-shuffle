...
const SavedStat = ({ stat, deleteStat }) => {
  ...
  const [gameNote, setGameNote] = useState(
    JSON.parse(localStorage.getItem(id)).gameNote
  );
  const [isSubmitted, setIsSubmitted] = useState(
    JSON.parse(localStorage.getItem(id)).isSubmitted
  );
    ...
    <div className="saved-stat">
      ...
      <div className="stats-container">
        <div className="bubble">
          {points > 9 ? (
            <p className="stat stat-blue">{points}</p>
          ) : (
        ...
        <div className="bubble">
          {rebounds > 9 ? (
            <p className="stat stat-blue">{rebounds}</p>
          ) : (
        ...
        <div className="bubble">
          {assists > 9 ? (
            <p className="stat stat-blue">{assists}</p>
          ) : (
      ...
      <div className="delete-button-container">
        <button className="delete-button" onClick={() => deleteStat(id)}>
          delete stat
        </button>
      </div>
    </div>
